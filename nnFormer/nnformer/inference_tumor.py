...
def test(fold):
    print("I am testing")
    path='/home/ubuntu/Desktop/nnFormer/nnformer/DATASET/test'
    label_list=sorted(glob.glob(os.path.join(path,'labelsTs','*nii.gz')))
    print(os.path.join(path,'labelsTs','*nii.gz'))
    ...
    for label_path,infer_path in zip(label_list,infer_list):
        ...
        print('dice_et testing: {:.4f}'.format(np.mean(Dice_et)))
        print('dice_tc testing: {:.4f}'.format(np.mean(Dice_tc)))
        print('dice_wt testing: {:.4f}'.format(np.mean(Dice_wt)))
    dsc=[]
    avg_hd=[]
    print("I am here")
