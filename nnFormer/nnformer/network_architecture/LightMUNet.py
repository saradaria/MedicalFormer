...
class LightMUNet(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        print("Initial input shape:", x.shape)  # Confirm initial shape

        # Initial convolution or operation
        x = self.convInit(x)
        print("After initial convolution:", x.shape)  # Check shape after first convolution

        if self.dropout_prob is not None:
            x = self.dropout(x)
            print("After dropout:", x.shape)  # Check shape after applying dropout

        down_x = []
        for idx, down in enumerate(self.down_layers):
            x = down(x)
            down_x.append(x)
            print(f"Shape after layer {idx}:", x.shape)  # Check shape after each layer in down_layers

            if idx == 0:  # Temporarily break after first layer to isolate the issue
                break
