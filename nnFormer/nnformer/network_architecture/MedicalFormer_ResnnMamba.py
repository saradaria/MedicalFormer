...
class MambaEncoder(nn.Module):
    def __init__(self,input_channels,channels,blocks):
        ...
        self.att4 = Attentionlayer(channels*8)
        self.layer4 = make_res_layer(channels * 8, channels * 16, blocks, stride=2, mamba_layer=MambaLayer(channels*16))
    def forward(self,x):
        outs = []
        #stem
        c1 = self.in_conv(x)
        ...
        c4 = self.layer3(c3)
        outs.append(c4)
        scale_f4 = self.att4(self.pooling(c4).reshape(c4.shape[0], c4.shape[1])).reshape(c4.shape[0], c4.shape[1], 1, 1, 1)
        c5 = self.layer4(c4)
        outs.append(c5)
...
class nnFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[64,128,128],
        ...
        self.encoder = MambaEncoder(input_channels,
                                    feat_size[0],
                                    blocks_mamba
                                    )
